...
(function () {
  ...
  /* Page-scoped bindings live in bindPage() so a single-file preview can
     re-run them after swapping routes. Global bindings above run once. */
  var stopHero = null;
  ...
  function bindPage() {
    ...
    if (stopHero) { stopHero(); stopHero = null; }
    var cv = $('#heroCanvas');
    if (cv && !reduce) {
      ...
      build(); start();
      var rt = null;
      var onResize = function () { clearTimeout(rt); rt = setTimeout(build, 180); };
